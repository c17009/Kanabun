...
public class Shooter : MonoBehaviour
{
    ...
    private void ShootFixedAngle(Vector3 aimpos, float aimangle)
    {
    ...
    private Vector3 ConvertVectorToVector3(float first_v, float first_angle, Vector3 aimpos)
    {
    ...
    private float ComputeVectorFormTime(Vector3 aimPos,float airTime)
    {
    ...
    private float ComputeAngleFromTime(Vector3 aimPos,float airTime)
    {
        Vector2 vec = ComputeVectorXYFromTime(aimPos, airTime);
        float v_x = vec.x;
        float v_y = vec.y;
    ...
    private Vector2 ComputeVectorXYFromTime(Vector3 aimPos,float airTime)
    {
        if(airTime <= 0.0f)
        {
            return Vector2.zero;
